...
class SkyblockPlayer:
    def __init__(
        ...
        *,
        ...
        self.profiles = []
        (
            self.latest_profile_name,
            self.latest_profile_index,
        ) = profile_info.get_latest_profile(self.skyblock_data, self.uuid)
        self.profile_name, self.profile_index = profile_info.get_profile_info(
            self.skyblock_data,
            profile_name,
            self.profiles,
            self.latest_profile_name,
        ...
        (
            self.rock_milestone,
            self.dolphin_milestone,
            self.next_rock_milestone,
            self.next_dolphin_milestone,
            self.rock_rarity,
            self.dolphin_rarity,
        ) = skyblock_data.get_pet_milestones(self.skyblock_data, self.profile_index, self.uuid)
        (
            self.currently_upgrading,
            self.coop_upgrades,
        ) = skyblock_data.get_community_upgrades(self.skyblock_data, self.profile_index)
        (
            self.jacob_medals,
            self.jacob_upgrades,
            self.jacob_info,
        ) = skyblock_data.get_jacob_data(self.skyblock_data, self.profile_index, self.uuid)
        (
            self.secrets,
            self.dungeon_class,
            self.cata_xp,
            self.cata_level,
            self.class_xp,
            self.dungeon_floor_info,
        ) = skyblock_data.get_dungeons_data(
            get_hypixel_data,
            self.skyblock_data,
            self.hypixel_data,
            self.profile_index,
        ...
        self.skills_info, self.total_xp = skyblock_data.get_skills_data(
            self.skyblock_data, self.profile_index, self.uuid
        )
        self.bazaar_info = {}
