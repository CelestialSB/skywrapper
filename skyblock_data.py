...
def get_money(skyblock_data, profile_index, uuid):
    ...
    bank_amount = (
        round(skyblock_data["profiles"][profile_index]["banking"]["balance"])
        if "banking" in skyblock_data["profiles"][profile_index]
        else "Bank API Disabled"
    )
...
def get_pet_milestones(skyblock_data, profile_index, uuid):
    ...
    next_rock_milestone = (
        [value for key, value in MILESTONE_TABLE["rock"].items() if value > rock_milestone][0]
        if len([value for key, value in MILESTONE_TABLE["rock"].items() if value > rock_milestone]) != 0
        else "Complete"
    )
    next_dolphin_milestone = (
        [value for key, value in MILESTONE_TABLE["dolphin"].items() if value > dolphin_milestone][0]
        if len([value for key, value in MILESTONE_TABLE["dolphin"].items() if value > dolphin_milestone]) != 0
        else "Complete"
    )
...
def get_community_upgrades(skyblock_data, profile_index):
    ...
    currently_upgrading = (
        upgrade_data["currently_upgrading"].get("upgrade").replace("_", " ").title()
        if "currently_upgrading" in upgrade_data and upgrade_data["currently_upgrading"] is not None
        else None
    )
    coop_upgrades = {
        upgrade_name: {
...
def get_jacob_data(skyblock_data, profile_index, uuid):
    ...
    jacobs_medals = {
        medal_name: (
            jacobs_data["medals_inv"][medal_name]
            if "medals_inv" in jacobs_data and medal_name in jacobs_data["medals_inv"]
            else medal_value
        )
        for medal_name, medal_value in JACOBS_MEDALS_TEMPLATE.items()
    }
    jacobs_upgrades = {
        upgrade_name: {
            "level": (
                jacobs_data["perks"][upgrade_name]
                if "perks" in jacobs_data and upgrade_name in jacobs_data["perks"]
                else JACOBS_UPGRADES_TEMPLATE[upgrade_name]["level"]
            ),
    ...
    jacobs_info = {
        item_id: {
            ...
            "gold_status": (
                True
                if ("unique_golds2" in jacobs_data and item_id in jacobs_data["unique_golds2"])
                or (
                    "unique_golds2" in jacobs_data
                    and item_id == "INK_SACK"
                    and "INK_SACK:3" in jacobs_data["unique_golds2"]
                )
                else False
            ),
            "highscore": max(
                [
                    event_value["collected"]
                    for event_name, event_value in jacobs_data.get("contests", {}).items()
                    if item_id in event_name
                ],
                default=0,
            ),
        }
...
def get_dungeons_data(get_hypixel_data, skyblock_data, hypixel_data, profile_index, uuid):
    ...
    cata_xp = (
        dungeon_data["dungeon_types"]["catacombs"]["experience"]
        if "dungeon_types" in dungeon_data and "experience" in dungeon_data["dungeon_types"]["catacombs"]
        else 0
    )
    cata_level = [key for key, value in DUNGEON_XP_TABLE.items() if cata_xp >= value][-1]
    class_xp = {
        class_name: (
            round(dungeon_data["player_classes"][class_name]["experience"])
            if "player_classes" in dungeon_data and "experience" in dungeon_data["player_classes"][class_name]
            else 0
        )
        for class_name in DUNGEON_CLASS_XP_TEMPLATE.keys()
    }
    dungeon_floor_info = {
        key: {
            "name": value["name"],
            "best_score": (
                dungeon_data["dungeon_types"]["catacombs"]["best_score"][key]
                if "dungeon_types" in dungeon_data
                and "best_score" in dungeon_data["dungeon_types"]["catacombs"]
                and key in dungeon_data["dungeon_types"]["catacombs"]["best_score"]
                else 0
            ),
            "fastest_time_s": (
                dungeon_data["dungeon_types"]["catacombs"]["fastest_time_s"][key]
                if "dungeon_types" in dungeon_data
                and "fastest_time_s" in dungeon_data["dungeon_types"]["catacombs"]
                and key in dungeon_data["dungeon_types"]["catacombs"]["fastest_time_s"]
                else 0
            ),
            "fastest_time_s_plus": (
                dungeon_data["dungeon_types"]["catacombs"]["fastest_time_s_plus"][key]
                if "dungeon_types" in dungeon_data
                and "fastest_time_s_plus" in dungeon_data["dungeon_types"]["catacombs"]
                and key in dungeon_data["dungeon_types"]["catacombs"]["fastest_time_s_plus"]
                else 0
            ),
            "completions": (
                dungeon_data["dungeon_types"]["catacombs"]["tier_completions"][key]
                if "dungeon_types" in dungeon_data
                and "tier_completions" in dungeon_data["dungeon_types"]["catacombs"]
                and key in dungeon_data["dungeon_types"]["catacombs"]["tier_completions"]
                else 0
            ),
        }
    ...
    slayer_info = {
        key: {
            ...
            "level": (
                [
                    template_key
                    for template_key, template_value in SLAYER_LEVEL[key].items()
                    if slayer_data[key].get("xp", 0) >= template_value
                ]
            )[-1],
        }
